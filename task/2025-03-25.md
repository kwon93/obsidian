...
###### 최종 테이블 생성 스크립트 
```
DROP TABLE IF EXISTS tra.t_pole; CREATE TABLE IF NOT EXISTS tra.t_pole ( id BIGSERIAL PRIMARY KEY, kkp FLOAT, kkpn FLOAT, tacho_count INT, latitude FLOAT, longitude FLOAT, type TEXT, sub_type INT, name TEXT, env_type TEXT, track_unit_id VARCHAR ); INSERT INTO tra.t_pole (kkpn, tacho_count, latitude, longitude, type, name, env_type) SELECT src.kp, src.tacho_count, src.latitude, src.longitude, 'P', src.pole_number, src.env_type FROM ( SELECT a.pole_number, a.tacho_count, a.kp, a.longitude, a.latitude, CASE WHEN (a.distance != 0 AND a.distance <= 3) OR a.next_distance <= 3 THEN 1 ELSE 0 END AS env_type FROM ( SELECT NULLIF(tpb.pole_number, '') AS pole_number, tpb.bracket_tacho::float AS tacho_count, tpb.kp::float AS kp, CASE WHEN tpb.tsi_lon IS NOT NULL AND tpb.tsi_lat IS NOT NULL THEN tpb.tsi_lon::float WHEN tpb.pds_lon IS NOT NULL AND tpb.pds_lat IS NOT NULL THEN tpb.pds_lon::float ELSE tpi.lsi_lon::float END AS longitude, CASE WHEN tpb.tsi_lon IS NOT NULL AND tpb.tsi_lat IS NOT NULL THEN tpb.tsi_lat::float WHEN tpb.pds_lon IS NOT NULL AND tpb.pds_lat IS NOT NULL THEN tpb.pds_lat::float ELSE tpi.lsi_lat::float END AS latitude, tpb.distance, LEAD(tpb.distance) OVER (ORDER BY tpb.kp) AS next_distance FROM tra.t_pole_bracket tpb LEFT JOIN tra.t_pole_image tpi ON tpb.image_index = tpi.image_index ) a ) src;
```
###### 중간 테이블 생성 스크립트 
